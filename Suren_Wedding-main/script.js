...
function startMusic() {
  music.play().then(() => {
    // Если запуск удался, обновляем кнопку
    toggleBtn.textContent = "🔊";
    // Удаляем слушателей, чтобы музыка не перезапускалась при каждом клике
    document.removeEventListener("click", startMusic);
    document.removeEventListener("touchstart", startMusic);
  }).catch(error => {
    // Автоплей все еще заблокирован, ждем взаимодействия
    console.log("Автозапуск заблокирован браузером. Ожидание клика...");
  });
}

// 1. Пытаемся запустить сразу (сработает, если юзер уже заходил и разрешил)
startMusic();

// 2. Ждем первого клика или касания в любом месте экрана
document.addEventListener("click", startMusic);
document.addEventListener("touchstart", startMusic);

// 3. Логика самой кнопки (оставляем вашу, но с e.stopPropagation)
toggleBtn.addEventListener("click", (e) => {
  e.stopPropagation(); // Чтобы клик по кнопке не вызывал startMusic еще раз
  if (music.paused) {
    music.play();
    toggleBtn.textContent = "🔊";
  } else {
